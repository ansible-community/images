#!/bin/bash
# Builds a Debian Trixie image suitable for use with ansible-test
...
build=$(buildah from docker.io/library/debian:trixie)
...
buildah commit "${build}" "${1:-localhost/test-image:debian-13-trixie}"
